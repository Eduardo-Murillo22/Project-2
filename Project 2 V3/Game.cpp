...
// Merge function for merging two halves of an array

void merge(Player players[], int left, int mid, int right, int playIdex[]) {
    int n1 = mid - left + 1;
    int n2 = right - mid;

    // Create temporary arrays
    Player* leftArray = new Player[n1];
    Player* rightArray = new Player[n2];

    // Copy data to temporary arrays leftArray[] and rightArray[]
    for (int i = 0; i < n1; i++) {
        leftArray[i] = players[playIdex[left + i]];
    }
    for (int j = 0; j < n2; j++) {
        rightArray[j] = players[playIdex[mid + 1 + j]];
    }

    // Merge the temporary arrays back into players[playIdex]
    int i = 0; // Initial index of first subarray
    int j = 0; // Initial index of second subarray
    int k = left; // Initial index of merged subarray

    while (i < n1 && j < n2) {
        if (leftArray[i].GetWin() > rightArray[j].GetWin()) {
            players[playIdex[k]] = leftArray[i];
            i++;
        } else {
            players[playIdex[k]] = rightArray[j];
            j++;
        }
        k++;
    }
...
void mergeSort(Player players[], int left, int right, int playIdex[]) {
    if (left < right) {
        // Same as (left+right)/2, but avoids overflow
        int mid = left + (right - left) / 2;

        // Sort first and second halves
        mergeSort(players, left, mid, playIdex);
        mergeSort(players, mid + 1, right, playIdex);

        // Merge the sorted halves
        merge(players, left, mid, right, playIdex);
    }
}
...
int Game::start(){
    ...
    for (int i = 0; i < Nplay; i++) {
        ...
        playerCardGraph.addPlayer(players[playIdex[i]].getName());
    }
    ...
    // Sort players based on win counts using merge sort
    mergeSort(players, 0, Nplay - 1, playIdex);
    cout << endl << endl;

    
    cout << "---------------Final Scores---------------" << endl;
    for (int i = 0; i < Nplay; i++) {
        cout << players[playIdex[i]].getName() << " has : " << players[playIdex[i]].GetWin() << " wins." << endl;
    }
...
void Game::Play(Player players[],int Nplay,int playIdex[]){
    ...
    for(int i = 0;i < Nplay;i++){
        //reset the values for cards on hand value
        playerCardGraph.addPlayer(players[playIdex[i]].getName());
        ...
        if(players[playIdex[i]].getMoney() < 0){
            ...
            cout << "You don't not have the money to play " << endl;
            PLAY = false;
    ...
    for(int i =0;i < Nplay;i++){
        ...
        if(players[playIdex[i]].getBet() >= 10){
            ...
            playerCardGraph.addCard(players[playIdex[i]].getName(), PlayerCardV[i]);
        }   
    }
                        ...
                        while (x == false) {                       
                            ...
                            switch (choice) {
                                case 1:{
                                    ...
                                    playerCardGraph.addCard(players[playIdex[i]].getName(), newCard);
    ...
    playerCardGraph.displayPlayerCards();
    cout << endl;
    ...
    for (int i = 0; i < Nplay; i++) {
        if (players[playIdex[i]].getBet() >= 10) {
            ...
            if (PlayerCardV[i] >= 22) {
                ...
                cout << players[playIdex[i]].getName() << ", Sorry You Lost you have " << PlayerCardV[i] << " and the dealer has " << dealerValue.front();
